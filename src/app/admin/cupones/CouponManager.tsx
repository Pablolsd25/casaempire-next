...
function describeValue(c: Coupon): string {
  ...
  if (c.type === 'fixed')         return `$${Number(c.value).toLocaleString('es-MX')} de descuento`
  return 'Envío gratis'
}
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end">
        <button onClick={openNew} className="btn-accent px-5 py-2.5 rounded text-sm flex items-center gap-1.5">
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-lg overflow-hidden">
        <div className="hidden md:grid grid-cols-[1.2fr_1.4fr_1fr_0.8fr_1fr_auto] gap-3 px-4 py-3 bg-zinc-950 text-zinc-500 text-xs border-b border-zinc-800">
          <span>Código</span><span>Descuento</span><span>Compra mín.</span>
          <span>Usos</span><span>Estado</span><span className="text-right">Acciones</span>
        </div>
        <div className="divide-y divide-zinc-800">
          {coupons.map((c) => {
            ...
            return (
              <div key={c.id} className="md:grid md:grid-cols-[1.2fr_1.4fr_1fr_0.8fr_1fr_auto] gap-3 px-4 py-3 items-center flex flex-wrap">
                <div>
                  <span className="font-mono font-bold text-accent text-sm">{c.code}</span>
                  {c.expires_at && (
                    <p className={`text-xs mt-0.5 ${expired ? 'text-red-400' : 'text-zinc-600'}`}>
                      {expired ? 'Expiró' : 'Vence'} {new Date(c.expires_at).toLocaleDateString('es-MX')}
                    </p>
                  )}
                </div>
                <span className="text-zinc-300 text-sm">{describeValue(c)}</span>
                <span className="text-zinc-400 text-sm">
                  {c.min_purchase ? `$${Number(c.min_purchase).toLocaleString('es-MX')}` : '—'}
                </span>
                <span className="text-zinc-400 text-sm">
                  {c.used_count}{c.max_uses != null ? ` / ${c.max_uses}` : ''}
                </span>
                <span>
                  <span className={`px-2 py-0.5 rounded text-xs font-medium ${active ? 'bg-green-500/20 text-green-400' : 'bg-zinc-700 text-zinc-400'}`}>
                    {active ? 'Activo' : c.is_active ? (expired ? 'Expirado' : 'Agotado') : 'Inactivo'}
                  </span>
                </span>
          ...
          {coupons.length === 0 && (
            <p className="px-5 py-12 text-center text-zinc-600 text-sm">Aún no hay cupones. Crea el primero.</p>
          )}
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[9999] p-4"
          ...
          <form onSubmit={handleSubmit} onClick={e => e.stopPropagation()}
            ...
            <div className="flex items-center justify-between">
              <h3 className="text-white font-bold text-lg">{isEdit ? 'Editar cupón' : 'Nuevo cupón'}</h3>
            ...
            {error && <div className="bg-red-500/10 border border-red-500/30 text-red-400 rounded p-2.5 text-sm">{error}</div>}

            {/* Código */}
            <div>
              <label className="block text-zinc-400 text-xs uppercase tracking-wide mb-1">Código</label>
              <input value={form.code} onChange={e => setForm(p => ({ ...p, code: e.target.value.toUpperCase() }))}
                required placeholder="VERANO20"
            ...
            {/* Tipo */}
            <div>
              <label className="block text-zinc-400 text-xs uppercase tracking-wide mb-1">Tipo de descuento</label>
              <select value={form.type} onChange={e => setForm(p => ({ ...p, type: e.target.value as CouponType }))}
            ...
            {form.type !== 'free_shipping' && (
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="number" step="0.01" min="0" max={form.type === 'percentage' ? '100' : undefined}
            ...
            <div className="grid grid-cols-2 gap-3">
              {/* Compra mínima */}
              <div>
                <label className="block text-zinc-400 text-xs uppercase tracking-wide mb-1">Compra mínima</label>
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-500 text-sm">$</span>
                  <input type="number" step="0.01" min="0" value={form.min_purchase}
                    onChange={e => setForm(p => ({ ...p, min_purchase: e.target.value }))} placeholder="0"
                    className="w-full bg-zinc-950 border border-zinc-700 text-white rounded-lg pl-8 pr-3 py-2.5 text-sm focus:outline-none focus:border-accent" />
                </div>
              </div>
              {/* Límite de usos */}
              <div>
                <label className="block text-zinc-400 text-xs uppercase tracking-wide mb-1">Límite de usos</label>
            ...
            {/* Expiración */}
            <div>
              <label className="block text-zinc-400 text-xs uppercase tracking-wide mb-1">Fecha de expiración (opcional)</label>
            ...
            <div className="flex gap-3 pt-1">
              <button type="submit" disabled={loading} className="btn-accent px-5 py-2.5 rounded text-sm disabled:opacity-50 flex-1">
